...
| 插件 | CraftEngine（版本以服务器 `/ce version` 与日志为准） | 自定义物品/方块/配方/家具 |
...
> 本章只说明数据包与 CE 的边界。CE 物品、方块和资源包的实际结构见 [CE 开发工作流](/develop/workflows/ce-workflow) 与 [CE 配置参考](/develop/workflows/ce-reference)。本章示例中的 namespace 只是示例，当前资源快照主要使用 `miragedge_items`、`miragedge_icon`、`miragedge_menu` 和 `customcrops`。
