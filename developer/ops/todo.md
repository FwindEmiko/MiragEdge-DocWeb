...
1. 只把条目 `status` 改为 `已放弃`，条目保留在 `groups[].items[]` 原位，不要剪切；
2. TodoList 组件会自动把它从待办区抽出，归入页面底部「已放弃（确定不做）」分区展示（弱化 + 删除线），不计入进行中/总计数量；
3. 想重新推进时把 `status` 改回原状态（如 `待开始`）即可。
...
- `卡住` 遇到了阻塞，需要讨论
- `构思中` 还没定具体方案
- `已放弃` 确定不再实现 / 不再推进（保留记录，自动归入页面底部「已放弃」分区，不参与待办计数）
