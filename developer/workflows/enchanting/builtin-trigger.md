---
title: "Builtin 触发器"
description: "使用 Java 编写 YAML 附魔的触发器"
---
...
按照 [事件函数索引](./event-functions) 使用准确的导入和签名。旧版本示例可能没有 `AiyatsbusEnchantment enchant` 参数，当前源码要求保留该参数。同一个回调可能为每个匹配的物品槽位各执行一次；如果玩家在多个匹配槽位上装备了该附魔，上述扣除饥饿值的效果也可能在同一轮执行多次。应通过附魔对象的 `active-slots` 限定生效槽位，或者在代码中明确处理 `slot`。不要在回调中重复注册全局监听器。
