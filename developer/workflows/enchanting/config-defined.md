---
title: "YAML 附魔"
description: "通过 YAML 管理附魔基础信息，使用 Java Builtin 或 Fluxon 编写效果。"
---
...
| 方式 | 效果实现 | 适用场景 | 需要阅读 |
| --- | --- | --- | --- |
| YAML + Builtin | Java `Builtin` 类 | 需要复杂 Java/Bukkit 逻辑，同时允许服主修改 YAML 参数 | [builtin-trigger](./builtin-trigger) |
| YAML + Fluxon | Listener、Ticker、Skill 的 Fluxon 脚本 | 希望附魔信息和效果都由 YAML 管理，不增加 Java 触发器类 | [mechanisms](./mechanisms) 和对应脚本触发器文档 |
...
`display.description` 必须遵循 [enchantment-fields](./enchantment-fields) 中的默认描述格式规范；如果开发者提供了自定义格式规范，则以开发者规范为准。
...
示例只展示内嵌脚本位置。实际 Bukkit getter、setter、枚举和变量用法必须遵循 [fluxon-language](./fluxon-language) 与 [fluxon-bukkit-java-semantics](./fluxon-bukkit-java-semantics)，并根据 Listener 的实际事件变量编写效果。
...
机制选择和公共字段见 [mechanisms](./mechanisms)：

- Bukkit 事件脚本：[listener-trigger](./listener-trigger)
- 周期脚本：[ticker-trigger](./ticker-trigger)
- 主动技能：[skill-trigger](./skill-trigger)
- Java 触发器：[builtin-trigger](./builtin-trigger)
- 粒子附魔：[artifact-trigger](./artifact-trigger)
...
不要因为数值固定就放入 ordinary，更不要直接写死在 Fluxon 或 Java 效果中。只有需要物品 NBT/PDC 持久化并在运行时修改的数据才使用 modifiable；布尔、字符串、枚举名、列表等非数值配置使用 ordinary。完整选择规则见 [enchantment-fields](./enchantment-fields)。

当前唯一脚本系统是 Fluxon，Listener、Ticker 和 Skill 通常省略 `type`，不要主动生成 `type: FLUXON`。生成脚本前必须阅读 [fluxon-language](./fluxon-language)。调用 Bukkit 或其他 Java API 时还必须阅读 [fluxon-bukkit-java-semantics](./fluxon-bukkit-java-semantics)。需要内置函数、扩展函数、JVM 互操作或模块时，再按需阅读 [fluxon-stdlib](./fluxon-stdlib)、[fluxon-jvm-interop](./fluxon-jvm-interop) 和 [fluxon-modules](./fluxon-modules)。Fluxon 只能直接嵌入机制的 YAML 脚本块，不能保存为独立 `.fs` 文件。

项目默认附魔 YAML 中存在部分历史 Fluxon 写法。它们只用于参考附魔结构和业务逻辑；新脚本必须以 [fluxon-language](./fluxon-language) 和 [fluxon-bukkit-java-semantics](./fluxon-bukkit-java-semantics) 为准，不要从默认资源复制与严格语义冲突的裸枚举或 getter 写法。
...
单文件重载会关闭旧机制并初始化新机制，但 Ticker 的玩家活跃状态 recorder 当前不会随关闭或全局 reset 一并清理。相同 Ticker ID 重载后，持续装备有效物品的玩家可能不会重新执行 `pre-handle`，关闭过程也不保证补发 `post-handle`。依赖进入或退出状态的脚本必须阅读 [ticker-trigger](./ticker-trigger) 的重载限制。
