---
title: "机制总览"
description: "Aiyatsbus 支持的 5 类附魔机制：Listener、Ticker、Skill、Builtin、Artifact。"
---
...
| --- | --- | --- | --- |
| Listener | `mechanisms.listeners` | 监听 Bukkit 事件并执行脚本 | [listener-trigger](./listener-trigger) |
| Ticker | `mechanisms.tickers` | 按固定 tick 间隔执行脚本 | [ticker-trigger](./ticker-trigger) |
| Skill | `mechanisms.skills` | 由左键、右键或切换主副手主动触发脚本 | [skill-trigger](./skill-trigger) |
| Builtin | `mechanisms.builtin` | 使用 Java 编写附魔效果 | [builtin-trigger](./builtin-trigger) |
| Artifact | `mechanisms.artifact` | 快速配置粒子附魔 | [artifact-trigger](./artifact-trigger) |
...
Builtin 和 Artifact 会按以下顺序读取概率变量：优先使用名为 `chance` 的 LEVELED 变量，其次使用 `概率`，都不存在时按 `100.0` 处理。Listener、Ticker 和 Skill 不会自动读取这两个变量；需要概率时必须在脚本中显式判断。概率值位于 `0.0..1.0` 时按比例解释，例如 `0.5` 是 50%；大于 `1.0` 时按百分数解释，例如 `25.5` 是 25.5%。当前执行器还会强制把概率结果转换为 `Double`，但 LEVELED 的整数结果会返回 `Int`；具体限制和示例见 [artifact-trigger](./artifact-trigger)。
...
所有脚本必须遵循 [fluxon-language](./fluxon-language)。调用 Bukkit 或其他 Java API 时还必须遵循 [fluxon-bukkit-java-semantics](./fluxon-bukkit-java-semantics)。需要函数、扩展 API、JVM 互操作或模块时，再分别读取 [fluxon-stdlib](./fluxon-stdlib)、[fluxon-jvm-interop](./fluxon-jvm-interop) 和 [fluxon-modules](./fluxon-modules)。
...
1. 当前开发者明确提供的项目规范。
2. [fluxon-language](./fluxon-language) 和 [fluxon-bukkit-java-semantics](./fluxon-bukkit-java-semantics)。
3. [aiyatsbus-fluxon-functions](./aiyatsbus-fluxon-functions)。
4. 默认附魔 YAML 中的历史示例。
