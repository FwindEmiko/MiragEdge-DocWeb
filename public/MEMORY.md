...
- MEMORY.md 正文里不能出现裸尖括号标签（如 script setup），会被 vue 编译器当成未闭合标签导致 vitepress build 失败；必须写成行内代码 `` 或用 HTML 实体转义
- layout.vue 的 `` 未标 lang="ts"，不能写 TS 类型注解（TS8010）；普通变量用无类型声明
