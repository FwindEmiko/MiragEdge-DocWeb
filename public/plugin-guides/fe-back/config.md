<!-- 来源页面: https://miragedge.top/plugin-guides/fe-back/config -->
<!-- 由 llms.txt 标准生成器自动产出，源文件: plugin-guides/fe-back/config.md -->
...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `currency-item.display-name` | String | `遗失的灵叶 ×%amount%` | 金粒悬浮名（MiniMessage）；`%amount%` = 本颗金粒代表的货币量（必须保留，否则不显示额度），`%owner%` = 持有人 UUID |
...
| `floodgate.form-title` | String | `死亡回程` | 表单标题（MiniMessage） |
| `floodgate.button-return` | String | `返回死亡点` | 返回按钮文字 |
| `floodgate.button-cancel` | String | `取消` | 取消按钮文字 |
...
文件位置：`plugins/FE_Back/messages.yml`，支持 MiniMessage 语法：``、``、``、``、`` 等。
