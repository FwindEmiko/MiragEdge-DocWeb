...
::: warning 即将启用
下面的碎片获取与合成方向已经确定；成品的名额解锁还在最终启用前验证。当前不要把右键成品当作已经可用的加名额方式，具体开放时间以游戏内提示和公告为准。
:::

想增加私人锚点或公开锚点的创建名额，需要前往对应维度的高危探索结构搜寻碎片。它们不是普通怪物掉落，也不能靠采矿或任意末地箱子获得。

**锚核碎片**出现在下界的烬域：下界反应堆的反应堆宝箱，以及圣所下层的宝箱，各有 **20%** 概率出现，每个宝箱最多获得 1 个。

**界标碎片**出现在末地终局探索：繁星末地的末地城飞船与凶兆宝库，各有 **15%** 概率出现，每次对应战利品结算最多获得 1 个。凶兆宝库需要按正常流程使用钥匙开启。

两类碎片都必须在工作台用完整的 3×3 格子合成：9 个锚核碎片合成 1 个**星辉锚点核心**，9 个界标碎片合成 1 个**星辉公开界标**。两种碎片不能混合使用。

启用后，星辉锚点核心会通过右键消耗，永久使自己的私人星辉锚点创建上限增加 1；星辉公开界标则永久使自己的公开星辉锚点上限增加 1。材料会被消耗，增加的名额属于使用者本人。
